...
function __print_git_status {
    local __git_status_added=0
    local __git_status_modified=0
    local __git_status_renamed=0
    local __git_status_deleted=0
    ...
    function __git_parse_status {
        ...
        [ $__git_status_added -gt 0 ] || \
            [ $__git_status_modified -gt 0 ] || \
            [ $__git_status_deleted -gt 0 ]
	...
	if [[ __is_git -eq 0 ]]; then
		...
		echo >&2 "I don't think this is a git repo."
	fi
}
